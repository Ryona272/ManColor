...
var PRESETS = {
  ...
  // AI先手特化: 全3回ちらちらを積極追求、マイナス色まで確定させて押し付ける
  senteStrategy: mergeParams({
    forceChirachiraThreshold: 3,
    chirachira1st: 50,
    chirachira2nd: 48,
    chirachira3rd: 40,
    chirachira1stMid: 50,
  ...
  // AI後手特化: 1回目ちらちらに全賭け、以降はぐるぐるで色予測を深める
...
function decideSpecialAction(state, memo, fortune, peeksDone, isOni = true, role = "opp", params = null) {
  ...
  if (isOni && peeksDone < (p.forceChirachiraThreshold ?? 2)) {
...
var fix3 = (v) => parseFloat(v.toFixed(3));
...
\u3010${label}\u3011`);
  const entries = PARAM_KEYS.filter(
    (k) => optimized[k] !== void 0 && optimized[k] !== initial[k]
...
var finalSG = runMany(sente, gote, N_VERIFY);
...
console.log(`
  vs DEFAULT(\u5148\u624B) \u6BD4\u8F03:`);
var finalDef = runMany(DEFAULT_PARAMS, DEFAULT_PARAMS, N_VERIFY);
console.log(
