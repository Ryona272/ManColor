...
function _pickPitWeighted(validPits, state, params, role) {
  ...
  const weights = validPits.map((pit) => {
    ...
    if (lastPit >= myLaneMin && lastPit <= myLaneMax && state.pits[lastPit].stones.length === 0) {
...
function _pickPlacementWeighted(myLaneMin, myLaneMax, myStore, state, params) {
  ...
  const weights = lanes.map((pit) => {
    ...
    return wouldGuru ? params.roboPlacementGuruBias ?? 1 : 1;
  });
  const total = weights.reduce((a, b) => a + b, 0);
  let r = Math.random() * total;
...
function runGameRoboVsOni(roboParams = DEFAULT_ROBO_PARAMS, roboRole = "opp", oniDepth = 3) {
...
function runManyRoboVsOni(roboParams = DEFAULT_ROBO_PARAMS, n = 500, oniDepth = 3) {
...
var RANGES = {
  roboGuruguruBias: [0.1, 20],
  // ぐるぐる着地pitを優先
  roboZakuzakuBias: [0.1, 20],
  // ざくざく着地pitを優先
  roboChirachiraBias: [0.1, 20],
  // ちらちら着地pitを優先
  roboPlacementGuruBias: [0.1, 20]
  // ざくざく後の配置: ぐるぐるセットアップ路を優先
};
var PARAM_KEYS = Object.keys(RANGES);
var WEAK_START = {
  roboGuruguruBias: 1,
  roboZakuzakuBias: 1,
  roboChirachiraBias: 1,
  roboPlacementGuruBias: 1
};
function mutate(params) {
  ...
  for (const key of PARAM_KEYS) {
    if (Math.random() < MUTATION_RATE) {
      ...
      child[key] = Number.isInteger(WEAK_START[key]) ? Math.round(v) : parseFloat(v.toFixed(4));
    }
  }
  return child;
}
...
function main() {
  ...
  console.log(
    `\u76EE\u6A19: 50%\u8D85\u3048 / \u4E0A\u9650 ${MAX_GENERATIONS} \u4E16\u4EE3 / \u53CE\u675F: ${CONVERGE_THRESHOLD}% \xD7 ${CONVERGE_PATIENCE}\u9023\u7D9A
`
  );
