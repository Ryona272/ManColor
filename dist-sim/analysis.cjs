...
function pickPit(role, validPits, state, memo, fortune, peeksDone, params = DEFAULT_PARAMS) {
  ...
  const playerLaneColors = /* @__PURE__ */ new Set();
  for (let q = oppLaneMin; q <= oppLaneMax; q++) {
    for (const s of state.pits[q].stones) playerLaneColors.add(s.color);
  }
  const playerLaneConsolidated = playerLaneColors.size > 0 && playerLaneColors.size <= 2;
  ...
  for (const p of validPits) {
    ...
    const playerThreatAfter = evalOppThreat(pitsAfter);
    {
      ...
      if (threatGrowth > 0)
        score -= threatGrowth * (params.playerThreatGrowthMult ?? 1);
    }
    {
    ...
    if (lastPit === storeIndex) {
      ...
      if (isEarlyGame) {
        score += params.guruguruBaseEarly * moodGuruguruMult + chainCount * chainCount * params.guruguruChainMultEarly;
        score += evalOwnFollowup(pitsAfter) * 0.8;
      } else {
        score += params.guruguruBase + chainCount * chainCount * params.guruguruChainMult;
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (landingPit >= laneMin && landingPit <= laneMax) {
        ...
        if (emptyPlayerPits.has(landedMirrorPlayer)) {
          ...
          score -= stonesInPit >= 3 ? stonesInPit * stonesInPit * 3 - 2 : 10 + stonesInPit;
        }
      }
    }
    for (let q = laneMin; q <= laneMax; q++) {
    ...
    for (const s of state.pits[p].stones) {
      ...
      if (playerAvoidedColor && s.color === playerAvoidedColor && !(ownFortune && s.color === ownFortune) && !knownPos.includes(s.color))
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (landingPit === storeIndex) {
        if (isEarlyGame) {
          ...
          if (playerAvoidedColor && stoneColor === playerAvoidedColor && !(ownFortune && stoneColor === ownFortune) && !knownPos.includes(stoneColor))
...
function pr(label, s) {
  ...
  const marker = parseFloat(s.selfWinRate) > parseFloat(s.oppWinRate) + 3 ? " \u25C0 self\u512A\u52E2" : parseFloat(s.oppWinRate) > parseFloat(s.selfWinRate) + 3 ? " \u25C0 opp\u512A\u52E2" : "";
...
var STEP1_CHAIN_QUAD = mergeParams({
  ...BASELINE,
  guruguruChainMult: 36,
  guruguruChainMultEarly: 9,
  guruguruBase: 77,
...
{
  for (const v of [0.5, 0.8, 1, 1.2, 1.5, 2]) {
