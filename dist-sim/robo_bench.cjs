...
var N_GAMES = 2e3;
var baseParams = { ...DEFAULT_ROBO_PARAMS };
try {
  const best = JSON.parse(import_fs.default.readFileSync("dist-sim/robo_best.json", "utf8"));
...
console.log(`\u958B\u59CB\u6642\u523B: ${(/* @__PURE__ */ new Date()).toLocaleTimeString()}
`);
var t0 = Date.now();
var result = runManyRoboVsOni(benchParams, N_GAMES);
var elapsed = ((Date.now() - t0) / 1e3).toFixed(1);
