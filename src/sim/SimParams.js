...
/**
 * testKyubi 用パラメータ（防御・妨害特化 AI）
 *
 * ピット選択スコア:
 *   - peeksDone===0 → pit5着地を強制（このパラメータは無効化不可）
 *   - ぐるぐる: tkGuruguruScore
 *   - ちらちら（2回目以降）: tkChirachiraScore
 *   - ざくざく: tkZakuzakuBase + captureCount
 *   - 相手ざくざくリスク: -tkOppZakuzakuPenalty * risk_stones
 *
 * 配置スコア（ざくざく後）:
 *   - ちらちらセットアップ路: tkChirachiraSetupBias 倍率
 *
 * こびふり（全確認後）:
 *   - AI占い色の石: tkPoipoiOwnFortune
 *   - 推測プレイヤー占い色: tkPoipoiInferred
 *   - 確認済み+1色: tkPoipoiKnownPos
 */
